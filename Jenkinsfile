...
pipeline {
    ...
    stages {
        stage('Cloning Git') {
            steps {
                checkout([$class: 'GitSCM', branches: [[name: '*/*']], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[credentialsId: '', url: 'https://github.com/Madan421/Demo']]])     
            }
        }
    ...
    // Building Docker images\
        ...
        stage ('sharing pkg to s3') {
            steps {
                script {
                    sh 'sudo chmod 777 /var/lib/jenkins/workspace/spring1code/*/.jar'
                    sh 'aws s3 cp /var/lib/jenkins/workspace/spring1code/*/.jar s3://hari220/spring1code'
                }
            }
        }

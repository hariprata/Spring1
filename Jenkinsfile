        ...
        stage ('sharing pkg to s3, build an image and push to ecr') {
            steps {
                script {
                    ...
                    sh 'sudo cd /root'
                    sh 'pwd'
                    sh 'sudo echo [hari] >> ~/.aws/credentials'
                    sh 'sudo echo "[profile hari]" >> ~/.aws/credentials'
                    sh 'sudo echo "region = us-east-1" >> ~/.aws/credentials'
                    sh 'sudo echo "aws_access_key_id = AKIA6LM5JB2DCI662XV4" >> ~/.aws/credentials'
                    sh 'sudo echo "aws_secret_access_key = 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" >> ~/.aws/credentials'
